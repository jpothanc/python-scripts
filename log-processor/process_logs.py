...
def write_bucket_summary_sheet(
    ...
    sheet.row_dimensions[table_start].height = 22
    ...
    def style_data_row(row_number: int, *, is_total: bool = False) -> None:
        ...
        for col in range(1, len(headers) + 1):
            ...
            style_cell(
                ...
                fill=row_fill,
                font=row_font,
                alignment=center if col > 1 else Alignment(horizontal="left", vertical="center", indent=1),
                border=SOFT_BORDER,
            )
    ...
    fit_column_widths(
        sheet,
        first_row=table_start,
        last_row=data_row - 1,
        min_widths=summary_table_min_widths(headers),
        max_width=55,
    )
